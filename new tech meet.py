...
##############################################################
def location(img_x,img_y,lat_x,log_y,box_x,box_y,ref_lat_x,ref_log_y,ratio):
    R = 6373.0
    dlog = log_y - ref_log_y   
    dlat = log_x - ref_log_x

    ax = sin(dlat / 2)**2 + cos(log_x) * cos(ref_log_x) * sin(0 / 2)**2
    cx = 2 * atan2(sqrt(ax), sqrt(1 - ax))

    gps_x = R * cx

    ay = sin(0 / 2)**2 + cos(lat_x) * cos(lat_x) * sin(dlon / 2)**2
    cy = 2 * atan2(sqrt(ay), sqrt(1 - ay))
...
#####################################################
    ...
    edge=cv2.addWeighted(img,0.7,new,0.3,0)
